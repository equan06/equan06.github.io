...
const dateConverter = (date) => {
  return DateTime.fromFormat(date, "MM-dd-yyyy").toFormat("MMM dd, yyyy");
};
</script>
  ...
  <div class="article-wrapper">
    ...
    >
      ...
      <div class="article-info">
        <span v-if="info.tag && info.tag.length > 0" class="tag"
          >Tag: {{ info.tag.join(", ") }}</span
        >
      </div>
      <div v-if="info.excerpt" class="excerpt" v-html="info.excerpt" />
      <div
        v-if="info.category && info.category.length > 0"
        class="category-container"
      >
        <span v-for="category in info.category" class="category-card">
          {{ category }}
        </span>
        <span v-if="info.date && !isTimeline" class="date"
          >{{ dateConverter(info.date) }}
        </span>
      </div>
    </article>
  </div>
