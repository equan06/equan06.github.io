...
Data was collected using python (with beautifulSoup4) and stored in an SQLite database for easy access. You can view the github repository (here)[https://github.com/equan06/london_marathon] for code (london.py), as well as the Jupyter Notebooks used for plot generation. Note: you have to reconfigure it every time for each gender/year/elite combination, since the HTML of the results page sometimes changes from year to year.
